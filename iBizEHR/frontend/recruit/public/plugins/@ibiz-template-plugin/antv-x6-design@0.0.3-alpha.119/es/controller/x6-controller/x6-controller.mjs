...
import { Transform } from '../../node_modules/.pnpm/@antv_x6-plugin-transform@2.1.8_@antv_x6@2.18.1/node_modules/@antv/x6-plugin-transform/es/index.mjs';
...
class X6Controller {
  ...
  constructor(panel, dom, minimap, opt = {}) {
    ...
    __publicField(this, "graphOptions", {
      ...
      connecting: {
        ...
        validateConnection: (args) => {
          if (args.targetCell) {
            const { inputNum } = args.targetCell;
            const edges = this.g.getConnectedEdges(args.targetCell);
            if (inputNum !== void 0 && edges.length >= inputNum)
              return false;
          }
          return true;
        },
        validateMagnet: (args) => {
          const { outputNum } = args.cell;
          const edges = this.g.getConnectedEdges(args.cell);
          if (outputNum !== void 0 && edges.length >= outputNum)
            return false;
          return true;
        }
      },
    ...
    this.link = new X6LinkController(panel, this.g, this);
    this.node = new X6NodeController(panel, this.g, this);
    this.init();
  }
  /**
   * 设置滚动条位置
