...
import { Trash } from '../x6-node-controller/icons/index.mjs';
...
class X6LinkController extends X6CellController {
   ...
   */
  contextMenu(e, edge) {
    const ns = useNamespace("x6-design");
  ...
  async load() {
    ...
    this.g.addEdges(
      this.items.map((item) => {
        ...
        return this.provider.createCell(data, this.context.designmode);
      })
    );
  }
  /**
  ...
  async createEdge(edge) {
    try {
      ...
      const cell = this.provider.createCell(
        dataProvider,
        this.context.designmode
      );
      const newEdge = this.g.addEdge(cell);
      this.g.resetSelection([newEdge]);
      this.x6.evt.emit("onLinkCreated", void 0);
    } catch (error) {
  ...
  async updateEdge(edge) {
    if (this.map.has(edge.id) === true) {
      ...
      this.x6.evt.emit("onLinkUpdated", void 0);
    }
  }
  /**
   * 删除连线数据
  ...
  async removeEdge(edge) {
    if (this.map.has(edge.id) === true) {
      ...
      if (data) {
        ...
        this.x6.evt.emit("onLinkRemoved", void 0);
        return true;
      }
    }
    return false;
