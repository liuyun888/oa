...
class X6NodeController extends X6CellController {
  ...
  initEvents() {
    ...
    g.on("node:selected", (args) => {
      const { node } = args;
      if (node.shape !== "remark-node" && this.context.designmode !== "STYLE2") {
  ...
  async createNode(node) {
    if (this.map.has(node.id) === false) {
      try {
        ...
        this.x6.evt.emit("onNodeCreated", void 0);
      } catch (error) {
  ...
  async updateNode(node) {
    if (this.map.has(node.id) === true) {
      ...
      this.x6.evt.emit("onNodeUpdated", void 0);
    }
  }
  /**
   * 删除节点数据
  ...
  async removeNode(node) {
    if (this.map.has(node.id) === true) {
      ...
      if (data) {
        ...
        this.x6.evt.emit("onNodeRemoved", void 0);
        return true;
      }
    }
    return false;
