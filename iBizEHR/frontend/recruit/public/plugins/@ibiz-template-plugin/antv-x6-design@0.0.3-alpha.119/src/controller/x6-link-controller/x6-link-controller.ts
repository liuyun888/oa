...
import { Trash } from '../x6-node-controller/icons';
...
export class X6LinkController extends X6CellController {
  ...
  async createEdge(edge: Edge): Promise<void> {
    try {
      ...
      this.x6.evt.emit('onLinkCreated', undefined);
    } catch (error) {
  ...
  async updateEdge(edge: Edge): Promise<void> {
    if (this.map.has(edge.id) === true) {
      ...
      this.x6.evt.emit('onLinkUpdated', undefined);
    }
  }
  ...
  async removeEdge(edge: Edge): Promise<boolean> {
    if (this.map.has(edge.id) === true) {
      ...
      if (data) {
        ...
        this.x6.evt.emit('onLinkRemoved', undefined);
        return true;
      }
    }
    return false;
