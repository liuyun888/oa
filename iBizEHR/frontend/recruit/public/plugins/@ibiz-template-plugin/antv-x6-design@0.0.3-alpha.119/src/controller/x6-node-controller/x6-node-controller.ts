...
export class X6NodeController extends X6CellController {
  ...
  protected initEvents(): void {
    ...
    // 监听备注节点值变更
    g.on('node:change:mome', ({ node }: IData): void => {
      fn(node);
    });
    // 监听节点大小改变
    g.on('node:change:size', ({ node, current }) => {
      node.trigger('resizing', current);
    });
    // 拖拽完成之后更新数据
    ...
    g.on('node:selected', args => {
      const { node } = args;
      if (
        node.shape !== 'remark-node' &&
        this.context.designmode !== 'STYLE2'
      ) {
  ...
  async createNode(node: Node): Promise<void> {
    if (this.map.has(node.id) === false) {
      try {
        ...
        this.x6.evt.emit('onNodeCreated', undefined);
      } catch (error) {
  ...
  async updateNode(node: Node): Promise<void> {
    if (this.map.has(node.id) === true) {
      ...
      this.x6.evt.emit('onNodeUpdated', undefined);
    }
  }
  ...
  async removeNode(node: Node): Promise<boolean> {
    if (this.map.has(node.id) === true) {
      ...
      if (data) {
        ...
        this.x6.evt.emit('onNodeRemoved', undefined);
        return true;
      }
    }
    return false;
