...
import { Graph, Options, Node } from '@antv/x6';
...
export class X6Controller {
  ...
  protected graphOptions: Graph.Options = {
    ...
    connecting: {
      ...
      allowEdge: false,
      allowBlank: false,
      validateConnection: (args: Options.ValidateConnectionArgs) => {
        if (args.targetCell) {
          const { inputNum } = args.targetCell as any;
          const edges = this.g.getConnectedEdges(args.targetCell);
          if (inputNum !== undefined && edges.length >= inputNum) return false;
        }
        return true;
      },
      validateMagnet: (args: any) => {
        ...
        if (outputNum !== undefined && edges.length >= outputNum) return false;
        return true;
      },
    },
    // 最小最大缩放级别
