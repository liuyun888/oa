  ...
  implements X6NodeServiceProvider
{
  ...
  async copy(
    ...
    const data = await this.get(context, params, node.id);
    const { x, y } = node.position();
    const { memo } = node.getData();
    const { width, height } = node.getSize();
    const deService = await this.getDeService(context);
  ...
  createCell(data: X6NodeDataProvider<IDataEntity>): Node.Metadata {
    if (data.type === 'TEXTAREA') return this.createRemarkCell(data);
    ...
    return {
      ...
      type: data.type,
      data,
      inputNum: (codeItem as any)?.intput_cnt,
      outputNum: (codeItem as any)?.output_cnt,
    };
  }

  createRemarkCell(data: X6NodeDataProvider<IDataEntity>): Node.Metadata {
    return {
      data,
      x: data.x,
      y: data.y,
