...
import './model-clipboard-import-view.css';

"use strict";
var IBizModelClipboardImportView = /* @__PURE__ */ defineComponent({
  ...
  setup(props) {
    ...
    const materialItem = computed(() => {
      return modelClipboardController.state.items.filter((item) => {
        if (item.type !== "default" || !Object.is(c.state.params.codeName, item.codeName) || c.state.items.findIndex((self) => Object.is(self.uuid, item.uuid)) !== -1) {
          return false;
        }
        return item;
      });
...
export { IBizModelClipboardImportView as default };
