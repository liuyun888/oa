...
1. 将`Default(传入变量).pssysbireportname(智能报表名称)` 设置给  `report(分析报表).NAME(名称)`
2. 将`Default(传入变量).PSSYSBIREPORTID(智能报表标识)` 设置给  `report(分析报表).ID(标识)`
...
调用实体 [分析报表(REPORT)](module/base/report.md) 行为 [Update](module/base/report#行为) ，行为参数为`report(分析报表)`
...
|分析报表|report|数据对象|[分析报表(REPORT)](module/base/report.md)||
