...
hide empty description
state "开始" as Begin <<start>> [[$./personal#begin {"开始"}]]
state "准备参数" as PREPAREPARAM1  [[$./personal#prepareparam1 {"准备参数"}]]
state "获取员工" as DEACTION1  [[$./personal#deaction1 {"获取员工"}]]
state "获取考勤记录" as RAWSFCODE1  [[$./personal#rawsfcode1 {"获取考勤记录"}]]
state "结束" as END1 <<end>> [[$./personal#end1 {"结束"}]]
...
1. 将`Default(传入变量).id(标识)` 设置给  `emp(人员).ID(主键)`
...
#### 获取考勤记录 :id=RAWSFCODE1<sup class="footnote-symbol"> <font color=gray size=1>[直接后台代码]</font></sup>
