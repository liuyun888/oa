<template>
	<section class="page-container border padding">
      ...
      <el-button icon="el-icon-download" style="margin-left: 0;margin-right: 13px;" v-loading="load.receive"
        :disabled="load.receive==true" @click="checkReceiveMail">收取邮件</el-button>
      ...
      <el-col class="itemTable">
        ...
        <el-table :height="tableHeight" ref="procdefsTable" :data="mailEmails" @sort-change="sortChange"
          ...
          <el-table-column prop="sendEmail" show-overflow-tooltip label="发件人邮箱" min-width="80"></el-table-column>
          ...
          <el-table-column prop="currentNum" show-overflow-tooltip label="序号" min-width="80"></el-table-column>
          <el-table-column prop="isStar" label="星标" min-width="80">
            <template slot-scope="scope">
              <i v-if="scope.row.isStar == '1'" @click.stop="setStar(scope.row)" style="color:#ffcc33;font-size:22px;"
          ...
          <el-table-column label="操作" width="80" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button type="danger" @click.stop="handleDel(scope.row,scope.$index)" icon="el-icon-delete"></el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-dialog title="收信" width="50%" :visible.sync="receiveVisible" :close-on-click-modal="false">
        <el-button type="danger" @click="receiveAllMail">收取全部</el-button>
        ...
        <el-drawer title="邮件详情" :visible.sync="emailInfoVisible" direction="rtl" size="50%">
          <div>
            <el-row>
              ...
              <el-col :span="20">{{emailInfo.sendEmail}}</el-col>
            </el-row>
            <el-row>
  ...
  export default {
    ...
    methods: {
      ...
      getMailEmails() {
        ...
        if (this.selectAccount != '' && this.selectAccount != '帐号列表') {
          params.receiveEmail = this.selectAccount
        }
      ...
      receiveAllMail() {
        ...
        let params = this.treeData[0].children
        receiveAllMailEmail(params).then((res) => {
          var tips = res.data.tips;
          if (tips.isOk) {
      ...
      setRead: function(row) {
        row.isRead = '1'
        let params = Object.assign({}, row);
        //params.isRead = '1'
        setReadMailEmail(params).then((res) => {
          var tips = res.data.tips;
          if (tips.isOk) {
            //this.getMailEmails();
          }
