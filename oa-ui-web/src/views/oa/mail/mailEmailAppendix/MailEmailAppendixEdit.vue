		 ...
		 <el-row class="page-container border padding"> 
			<el-form :model="editForm"  label-width="120px" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="邮件id" prop="emailId">
					<el-input v-model="editForm.emailId" placeholder="邮件id"></el-input>
				</el-form-item> 
...
<script>
	...
	import { editMailEmailAppendix } from '@/api/oa/mail/mailEmailAppendix';
	...
	export default { 
		...
		watch: {
	      'mailEmailAppendix':function( mailEmailAppendix ) {
	        this.editForm = mailEmailAppendix;
	      },
		...
		data() {
			return {
				...
				//编辑界面数据  MailEmailAppendix mail_email_appendix
				editForm: {
		...
		methods: {
			...
			//编辑提交MailEmailAppendix mail_email_appendix父组件监听@submit="afterEditSubmit"
			editSubmit: function () {
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => { 
							...
							editMailEmailAppendix(params).then((res) => {
		...
		mounted() {
			this.editForm=Object.assign(this.editForm, this.mailEmailAppendix);  
		}
	}
