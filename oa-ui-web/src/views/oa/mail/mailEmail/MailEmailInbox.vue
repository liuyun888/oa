<template>
	<section class="page-container border padding">
      ...
      <el-select v-model="checkWho" @change="onChange" placeholder="请选择">
        <el-option label="我的收件" value="0"></el-option>
        <el-option label="我的抄送" value="1"></el-option>
        <el-option label="我的密送" value="2"></el-option>
      </el-select>
      ...
      <el-button icon="el-icon-s-promotion" style="margin-left: 0;margin-right: 13px;" @click="showAdd('0')">发送邮件
      </el-button>
      ...
      <!-- <el-row>
        <el-radio-group v-model="checkWho" @change="onChange">
          <el-radio-button label="0">我的收件</el-radio-button>
          <el-radio-button label="1">我的抄送</el-radio-button>
          <el-radio-button label="2">我的密送</el-radio-button>
        </el-radio-group>
      </el-row> -->
      ...
      <el-table :height="tableHeight" ref="procdefsTable" :data="mailEmails" @sort-change="sortChange"
        ...
        <el-table-column prop="meStar" label="星标" min-width="80">
          <template slot-scope="scope">
            <i v-if="scope.row.meStar == '1'" @click.stop="setStarAddessee(scope.row)"
              style="color:#ffcc33;font-size:22px;" class="el-icon-star-on"></i>
            <i v-else style="font-size:22px;" @click.stop="setStarAddessee(scope.row)" class="el-icon-star-on"></i>
          </template>
        </el-table-column>
        <el-table-column prop="isRead" label="是否已读" min-width="80">
          <template slot-scope="scope">
            <el-tag type='info' v-if="scope.row.meRead == '1'">是</el-tag>
        ...
        <el-table-column label="操作" width="80" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button type="danger" @click.stop="handleDelAddress(scope.row,scope.$index)" icon="el-icon-delete">
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  import {
    ...
    delMailEmailAddressee,
    setReadMailEmailAddressee
  ...
  export default {
    ...
    methods: {
      ...
      getMailEmails() {
        ...
        if (this.currentAddressee != '') {
          params.currentAddressee = this.currentAddressee
        } else if (this.currentCc != '') {
          params.currentCc = this.currentCc
        } else if (this.currentBcc != '') {
          params.currentBcc = this.currentBcc
        }
      ...
      setReadMailEmail(row) {
        row.meRead = '1'
        let params = {}
        params.id = row.meId
        params.isRead = row.meRead
        setReadMailEmailAddressee(params).then((res) => {
          var tips = res.data.tips;
          if (tips.isOk) {
            this.getMailEmails();
          }
      ...
      onChange(value) {
        ...
        this.currentBcc = ''
        if (value == '2') {
          this.currentBcc = this.userInfo.userid
        } else if (value == '1') {
          this.currentCc = this.userInfo.userid
        } else {
          this.currentAddressee = this.userInfo.userid
        }
        this.getMailEmails();
      },
      /**end 自定义函数请在上面加**/
