<template>
	<section class="page-container border padding">
		<el-row class="page-header">
			...
			<el-button type="primary" icon="el-icon-search" style="margin-right: 13px;" v-loading="load.list" :disabled="load.list==true" v-on:click="searchMailEmails">查询</el-button>
			<!-- <el-button type="primary" @click="showAdd('1')">添加外部邮件</el-button> -->
			<el-button icon="el-icon-s-promotion" style="margin-left: 0;margin-right: 13px;"  @click="showAdd('0')">发送邮件</el-button>
			<el-button type="danger" icon="el-icon-delete" style="margin-left: 0;" v-loading="load.del" @click="batchDel" :disabled="this.sels.length===0 || load.del==true">批量删除</el-button>
		</el-row>
		<el-row class="page-main padding-top">
			<!--列表 MailEmail mail_email-->
			<el-table :height="tableHeight" ref="procdefsTable" :data="mailEmails" @sort-change="sortChange" highlight-current-row v-loading="load.list" border @selection-change="selsChange" @row-click="rowClick" style="width: 100%;">
				...
				<!-- <el-table-column prop="content" label="正文" min-width="80" ></el-table-column>
				<el-table-column prop="senderId" label="发件人" min-width="80" ></el-table-column> -->
				<el-table-column prop="username" show-overflow-tooltip label="发件人" min-width="80" ></el-table-column>
				<!-- <el-table-column prop="sendEmail" label="发件邮箱" min-width="80" ></el-table-column> -->
				...
				<template slot-scope="scope">
					<i v-if="scope.row.isStar == '1'" @click.stop="setStar(scope.row)" style="color:#ffcc33;font-size:22px;" class="el-icon-star-on"></i>
				...
				<el-table-column label="操作" width="200" fixed="right" >
					<template slot-scope="scope">
						<el-button  @click.stop="showEdit( scope.row,scope.$index)">编辑发送</el-button>
				...
				<el-drawer title="邮件详情" :visible.sync="emailInfoVisible" direction="rtl" size="50%">
					<div >
						...
						<el-row>
							...
							<el-col v-if="emailInfo.status == '0'" :span="20">未发送</el-col>
						</el-row>
						<el-row>
						...
						<el-row>
							...
							<el-col :span="20"><div v-html="emailInfo.content"></div></el-col>
						</el-row>
			...
			<el-dialog title="发送邮件" :visible.sync="editFormVisible"  width="50%"  :close-on-click-modal="false">
				  <mail-email-edit :mail-email="editForm" :visible="editFormVisible" @cancel="editFormVisible=false" @submit="afterEditSubmit"></mail-email-edit>
			</el-dialog>

			<!--新增 MailEmail mail_email界面-->
			<el-dialog title="新增邮件" :visible.sync="addFormVisible"  width="60%"  :close-on-click-modal="false">
				<mail-email-add :mail-email="addForm" :is-outer="isOuter" :visible="addFormVisible" @cancel="addFormVisible=false" @submit="afterAddSubmit"></mail-email-add>
			</el-dialog>
		</el-row>
	</section>
</template>
...
<script>
	...
	import { listMailEmail, delMailEmail, batchDelMailEmail,setStarMailEmail } from '@/api/oa/mail/mailEmail';
	import  MailEmailAdd from './MailEmailAdd';//新增界面
	import  MailEmailEdit from './MailEmailEdit';//修改界面
	import { setStarMailEmailAddressee } from '@/api/oa/mail/mailEmailAddressee';
	...
	export default {
		...
		data() {
			return {
				...
				emailInfo: {
					id: '',subject:'',content:'',userId:'',username:'',sendEmail:'',sendTime:'',isStar:'',isRead:'',status:'',branchId:'',addressee:'',cc:'',bcc:'',
					addresseeList:[],
					ccList:[],
					bccList:[],
					appendixList:[],
