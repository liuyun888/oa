...
<script>
	...
	import { addMeetMeetingAttender } from '@/api/oa/meet/meetMeetingAttender';
	...
	export default { 
		...
		watch: {
	      'meetMeetingAttender':function( meetMeetingAttender ) {
	        this.addForm = meetMeetingAttender;
	      },
		...
		methods: {
			...
			//新增提交MeetMeetingAttender meet_meeting_attender 父组件监听@submit="afterAddSubmit"
			addSubmit: function () {
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						...
						this.$confirm('确认提交吗？', '提示', {}).then(() => { 
							...
							addMeetMeetingAttender(params).then((res) => {
								this.load.add=false
		...
		mounted() {
			this.addForm=Object.assign(this.addForm, this.meetMeetingAttender);  
			/**在下面写其它函数***/
