<template>
  <section>
    <el-row>
      <el-row>
        <!--新增/编辑界面 AssetDetail ass_asset_detail-->
        <el-form :model="editForm" label-width="120px" :rules="editFormRules" ref="editFormRef">
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="资产原值" prop="orignUnitAmount">
                <el-select v-model="editForm.currency" placeholder="资产原值" :maxlength="50"
                           @change="editSomeFields(editForm,'orignUnitAmount',$event)"
                           :disabled="  disabledJudge('orignUnitAmount')">
                  <el-option v-for="item in currencyArr" :key="item.optionValue" :label="item.optionName"
                             :value="item.optionValue">
                  </el-option>
                </el-select>
                <!--               @focus="setHaoCaiCurrentRow(row.$index)" 没有实现 -->
          ...
          <el-row>
            ...
            <el-col :span="12">
              <el-form-item label="使用部门名称" prop="deptName">
                <mdp-select-dept v-model="editForm.deptName" placeholder="使用部门名称" :maxlength="50" :width="100+'%'"
                                 @change2="(option)=>{
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="存放地点" prop="storeAddress">
                <!--                <el-input v-model="editForm.storeAddress" placeholder="存放地点" :maxlength="50"-->
                <!--                          @change="editSomeFields(editForm,'storeAddress',$event)"-->
                <!--                          :disabled="  disabledJudge('storeAddress')"/>-->
                <el-input suffix-icon="el-icon-collection-tag" @focus="openWarehouseSelect" placeholder="存放地点"
                          :maxlength="50"
                          :disabled="  disabledJudge('storeAddress')" v-model="editForm.warehouseName"/>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="责任人" prop="personLiableUserid">
                <mdp-select-user v-model="editForm.personLiableUserid" placeholder="责任人" :maxlength="50"
                                 :width="100+'%'"
                                 @change2="(option)=>{
                                        editSomeFields(editForm,'personLiableUsername',option.name);
                                        editSomeFields(editForm,'personLiableUserid',option.id); }"
                                 :disabled="  disabledJudge('personLiableUserid')"/>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="所属父资产" prop="passetName">
                <el-input v-model="editForm.passetName" placeholder="所属父资产" :maxlength="50"
                          @focus="visibleAssetSelect=true" readonly
                          @change="editSomeFields(editForm,'passetName',$event)"
                          :disabled="  disabledJudge('passetName')"/>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="所属父资产编码" prop="passetSn">
                <el-input v-model="editForm.passetSn" placeholder="所属父资产资产编码" :maxlength="50"
                          @focus="visibleAssetSelect=true" readonly
                          :disabled=" disabledJudge('passetSn')"/>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
export default {
  ...
  data() {
    return {
      ...
      //增删改查(含批量)接口
      apis: {
        queryById: AssetDetailApi.queryAssetDetailById,
  ...
  methods: {
    getOption() {
      ...
      listOption(params).then(res => {
        this.currencyArr = res.data.data && res.data.data.currency ? res.data.data.currency : "";
      });
    },
