<template>
  <section class="page-container border padding">
    <el-row class="page-header">
      ...
      <el-input v-model="filters.id" style="width: 20%;" placeholder="申请单编号 查询 输入 *字符* >10 <9 等"
                clearable title="支持>、<、 >=、<=、!=、*字符*、$IS NULL、$IN 1,2,3、$between 1,5等操作符"/>
    ...
    <el-row>
      ...
      <el-table :ref="refId+'Table'" :height="100" v-adaptive="{bottomOffset: bottomOffset}" :data="tableDatas"
        ...
        <el-table-column prop="reqTitle" label="标题" min-width="120" show-overflow-tooltip col-type="String"
        ...
        <el-table-column prop="reqUsername" label="申请人" min-width="120" show-overflow-tooltip col-type="String"
        ...
        <el-table-column prop="mngStatus" label="状态" min-width="120" show-overflow-tooltip col-type="String"
                         v-if="showCol('mngStatus')">
          <template slot-scope="scope">
            <mdp-input show-style="tag" v-model="scope.row.mngStatus" :maxlength="10"
                       @change="editSomeFields(scope.row,'mngStatus',$event)"
                       :disabled="!editable || disabledJudge('mngStatus')">
...
export default {
  ...
  data() {
    ...
    fromStartTime.setHours(0);
    fromStartTime.setMinutes(0);
    fromStartTime.setSeconds(0);
    const toStartTime = new Date();
    toStartTime.setHours(23);
    toStartTime.setMinutes(59);
    toStartTime.setSeconds(59);
    ...
    return {
      ...
      filters: {//查询参数
        reqDate: [util.formatDate.format(fromStartTime, 'yyyy-MM-dd hh:mm:ss'), util.formatDate.format(toStartTime, 'yyyy-MM-dd hh:mm:ss')],
      },
