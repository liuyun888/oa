	...
	<section class="page-container border padding">
		...
		<el-row class="page-main padding-top">
			...
			<el-table :height="tableHeight" ref="procdefsTable"  @cell-click="cellClick" :data="assetDetails" @sort-change="sortChange" highlight-current-row v-loading="load.list" border @selection-change="selsChange" @row-click="rowClick" style="width: 100%;">
				...
				<el-table-column prop="reqTitle" label="标题"  show-overflow-tooltip>
				</el-table-column>
            ...
            <el-tag  v-if="scope.row.mngStatus == 'hold'">未发审</el-tag>
            <el-tag  v-if="scope.row.mngStatus == 'submit'">审核中</el-tag>
            <el-tag  v-if="scope.row.mngStatus == 'permit'">已归还</el-tag>
            <el-tag  v-if="scope.row.mngStatus == 'waiting'">待归还</el-tag>
            <el-tag  v-if="scope.row.mngStatus == 'cancel'">已取消</el-tag>
            <el-tag  v-if="scope.row.mngStatus == 'reject'">未通过</el-tag>
					</template>
				</el-table-column>
			...
			<el-dialog title="编辑归还单" center :fullscreen="true" :visible.sync="editFormVisible" @close="addClose"  :close-on-click-modal="false">
				  <asset-return-edit ref="assetReturn" :asset-return="editForm" :visible="editFormVisible" @cancel="editFormVisible=false" @submit="afterEditSubmit"></asset-return-edit>
			</el-dialog>
			...
			<el-dialog title="归还明细" center :fullscreen="true" :visible.sync="detailFormVisible" @close="addClose" :close-on-click-modal="false">
				<asset-return-detail ref="assetReturn" :asset-receive="detailForm" :visible="detailFormVisible" @cancel="detailFormVisible=false"></asset-return-detail>
			</el-dialog>
...
<script>
	...
	import assetReturnAdd from './assetReturnAdd';
	import assetReturnDetail from './assetReturnDetail';
	import assetReturnEdit from './assetReturnEdit';
  ...
  import config from "@/common/config"; //全局公共库
	...
	export default {
		...
		methods: {
			...
			showEdit: function ( row,index ) {
				if(row.mngStatus === 'submit'||row.mngStatus === 'waiting'||row.mngStatus === 'permit') {
					this.$message({ message: "该单已提交，不能修改", type: 'error' });
					return;
				}
			...
			addClose() {
				this.$refs.assetReturn.addClose();
			},

			afterEditSubmit(val1){
				this.editFormVisible=false;
				this.getAssetMng();
        this.sendToProcessApprova(val1,"asset_return_process_approva");
			},
			//选择行assetDetail
			...
			handleDel: function (row,index) {
				if(row.mngStatus === 'submit'||row.mngStatus === 'waiting'||row.mngStatus === 'permit') {
					this.$message({ message: "该单已提交，不能删除", type: 'error' });
					return;
				}
      ...
      sendToProcessApprova:function(row,bizKey){
        ...
        var titleNames=row.assetMng.reqUsername
        var mainTitle=''
        var mainContext=''
        if(bizKey=='asset_return_process_approva'){
          mainTitle='关于【'+titleNames+"归还资产】的审批"
          mainContext=mainTitle+',所在部门：'+row.assetMng.reqDeptName+",申请事由："+row.assetMng.reqReason;
        }else{
        ...
        let params={
          ...
          //restUrl:"http://localhost:7078"+"/erp/ass/assetMng/returnProcessApprova",
          extVars:extVars,
          flowVars:{
		...
		components: {
			...
			'asset-return-edit' : assetReturnEdit,
		},
		mounted() {
			//默认查询领用类型的数据
			this.$nextTick(() => {
