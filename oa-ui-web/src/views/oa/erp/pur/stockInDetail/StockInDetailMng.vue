<template>
  <section class="page-container border padding">
    <el-row>
      ...
      <el-button v-loading="load.list" :disabled="load.list==true" @click="searchStockInDetails" icon="el-icon-search">
        查询
      </el-button>
    ...
    <el-row class="padding-top">
      <!--列表 StockInDetail pur_stock_in_detail-->
      <el-table ref="stockInDetailTable" :data="stockInDetails" :height="maxTableHeight" @sort-change="sortChange"
        ...
        <el-table-column prop="purchaseDate" label="采购日期" min-width="80" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column prop="inNum" label="入库数量" min-width="80" show-overflow-tooltip></el-table-column>
    ...
    <el-row>
      <el-dialog title="编辑入库单" center :fullscreen="true" :visible.sync="editFormVisible"
        ...
        <stock-in-edit :stock-in="stockIn" op-type="edit" :visible="editFormVisible" @cancel="editFormVisible=false"
...
import {listStockInDetail, delStockInDetail, batchDelStockInDetail} from '@/api/oa/erp/pur/stockInDetail';
import StockInEdit from '../stockIn/StockInEdit';//新增修改界面
...
export default {
  ...
  data() {
    return {
      ...
      addFormVisible: false,//新增stockInDetail界面是否显示
      addForm: {
        assetSn: '',
        categoryId: '',
        categoryName: '',
  ...
  methods: {
    ...
    //获取列表 StockInDetail pur_stock_in_detail
    getStockInDetails() {
      let params = {
      ...
      this.load.list = true;
      listStockInDetail(params).then((res) => {
        var tips = res.data.tips;
        if (tips.isOk) {
          this.pageInfo.total = res.data.total;
          this.pageInfo.count = false;
          this.stockInDetails = res.data.data;
        } else {
    ...
    //显示编辑界面 StockInDetail pur_stock_in_detail
    showEdit: function (row, index) {
      this.editFormVisible = true;
      this.editForm = Object.assign({}, row);
      this.stockIn = Object.assign({}, row)
      this.stockIn.id = this.stockIn.receiptNo
    },
    //显示新增界面 StockInDetail pur_stock_in_detail
    showAdd: function () {
    ...
    handleDel: function (row, index) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        this.load.del = true;
        let params = {assetSn: row.assetSn};
        delStockInDetail(params).then((res) => {
          this.load.del = false;
          var tips = res.data.tips;
          if (tips.isOk) {
            this.pageInfo.count = true;
    ...
    batchDel: function () {
      ...
      this.$confirm('确认删除选中记录吗？', '提示', {
        ...
        this.load.del = true;
        batchDelStockInDetail(this.sels).then((res) => {
          this.load.del = false;
          var tips = res.data.tips;
          if (tips.isOk) {
            this.pageInfo.count = true;
