<template>
  <section class="page-container border padding">
      ...
      <el-popover title="更多查询条件" placement="top-start" width="400" trigger="click">
        <el-row class="more-filter-item">
          ...
          <el-col :span="24" style="margin-top: 11px;">
            <el-button size="mini" type="primary" v-on:click="searchStockIns" icon="el-icon-search">查询</el-button>
          </el-col>
    ...
    <el-row class="page-main padding-top">
      ...
      <el-table :height="tableHeight" ref="procdefsTable" @cell-click="cellClick" :data="stockIns"
        ...
        <el-table-column prop="title" label="标题" min-width="80" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column prop="inNum" label="入库总数量" min-width="80" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column label="操作" width="180" fixed="right" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-button type="text" :disabled="scope.row.bizFlowState=='1'||scope.row.bizFlowState=='2'"
                       icon="el-icon-edit" @click="sendToProcessApprova( scope.row,scope.$index)">发审
            </el-button>
            <el-button type="text" icon="el-icon-edit" @click="showEdit( scope.row,scope.$index)"></el-button>
            <el-button type="text" icon="el-icon-delete"
                       :disabled="scope.row.bizFlowState=='1'||scope.row.bizFlowState=='2'"
      ...
      <!--编辑 StockIn pur_stock_in界面-->
      <el-dialog title="编辑入库单" center :fullscreen="true" :visible.sync="editFormVisible"
                 :close-on-click-modal="false" append-to-body>
        <stock-in-edit :stock-in="editForm" op-type="edit" :visible="editFormVisible" @cancel="editFormVisible=false"
                       @submit="afterEditSubmit"></stock-in-edit>
      </el-dialog>

      <!--新增 StockIn pur_stock_in界面-->
      <el-dialog title="新增入库单" center :fullscreen="true" :visible.sync="addFormVisible" @close="addFromClose"
                 :close-on-click-modal="false" append-to-body>
        <stock-in-edit ref="stockInAdd" op-type="add" :stock-in="addForm" :visible="addFormVisible"
                       @cancel="addFormVisible=false" @submit="afterAddSubmit"></stock-in-edit>
      </el-dialog>
...
import {listStockIn, delStockIn, batchDelStockIn} from '@/api/oa/erp/pur/stockIn';
import StockInEdit from './StockInEdit';//修改界面
...
export default {
  ...
  data() {
    ...
    return {
      ...
      addFormVisible: false,//新增stockIn界面是否显示
      //新增stockIn界面初始化数据
      addForm: {
        id: '',
        title: '',
  ...
  methods: {
    ...
    getStockIns() {
      ...
      this.load.list = true;
      listStockIn(params).then((res) => {
        var tips = res.data.tips;
        if (tips.isOk) {
          this.pageInfo.total = res.data.total;
          this.pageInfo.count = false;
          this.stockIns = res.data.data;
        } else {
    ...
    //显示编辑界面 StockIn pur_stock_in
    showEdit: function (row, index) {
      this.editFormVisible = true;
      this.editForm = Object.assign({}, row);
    },
    //显示新增界面 StockIn pur_stock_in
    showAdd: function () {
      this.addFormVisible = true;
      //this.addForm=Object.assign({}, this.editForm);
    },
    afterAddSubmit(row, isFlow) {
      this.addFormVisible = false;
      this.pageInfo.count = true;
      this.getStockIns();
      if (isFlow) {
        sendStockInToProcessApprova(this, row)
      }
    },
    afterEditSubmit(row, isFlow) {
    ...
    batchDel: function () {
      ...
      this.$confirm('确认删除选中记录吗？', '提示', {
        ...
        this.load.del = true;
        batchDelStockIn(this.sels).then((res) => {
          this.load.del = false;
          var tips = res.data.tips;
          if (tips.isOk) {
            this.pageInfo.count = true;
    ...
    /**begin 自定义函数请在下面加**/
    addFromClose: function () {
      this.$refs.stockInAdd.cancel();
    },

    detailClose: function () {
      this.$refs.stockInDetail.cancel();
    },
