<template>
  <section class="border padding">
    <el-row>
      ...
      <el-input v-model="filters.title" style="width: 20%;" placeholder="标题查询 输入 *字符* >10 <9 等" clearable
                title="支持>、<、 >=、<=、!=、*字符*、$IS NULL、$IN 1,2,3、$between 1,5等操作符"/>

      <el-input v-model="filters.id" style="width: 20%;" placeholder="入库单号查询 输入 *字符* >10 <9 等" clearable
                title="支持>、<、 >=、<=、!=、*字符*、$IS NULL、$IN 1,2,3、$between 1,5等操作符"/>

      <mdp-select-user style="width: 20%;" placeholder="申请人查询" clearable
                       @change2="(option)=>{
                        filters.reqUserid = option.id;
                        filters.reqUsername = option.name;}"/>
    ...
    <el-row>
      ...
      <el-table :ref="refId+'Table'" :height="100" v-adaptive="{bottomOffset: bottomOffset}" :data="tableDatas"
        ...
        <el-table-column prop="title" label="入库单标题" min-width="120" show-overflow-tooltip col-type="String"
                         v-if="showCol('title')">
        ...
        <el-table-column prop="id" label="入库单号" min-width="120" show-overflow-tooltip fixed="left" col-type="String"
                         v-if="showCol('id')"></el-table-column>
        <el-table-column prop="bizFlowState" label="审核状态" min-width="120"
        ...
        <el-table-column prop="totalInNum" label="总入库数量" min-width="120" show-overflow-tooltip
                         col-type="BigDecimal" v-if="showCol('totalInNum')">
          <template slot-scope="scope">
            <mdp-input show-style="tag" v-model="scope.row.totalInNum"
                       @change="editSomeFields(scope.row,'totalInNum',$event)" :maxlength="15"
                       :disabled="!editable || disabledJudge('totalInNum')"/>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="stockStatus" label="入库单状态" min-width="120" show-overflow-tooltip col-type="String"
                         v-if="showCol('stockStatus')">
          <template slot-scope="scope">
            <mdp-input show-style="tag" v-model="scope.row.stockStatus" :maxlength="10"
                       @change="editSomeFields(scope.row,'stockStatus',$event)"
                       :disabled="!editable || disabledJudge('stockStatus')">
              <span>{{ scope.row.stockStatus === '1' ? '已入库' : '未入库' }}</span>
            </mdp-input>
          </template>
        </el-table-column>

        <el-table-column label="操作" :width="currOpType=='mng'?220:100" fixed="right">
          ...
          <template scope="scope" v-if="currOpType=='mng'">
            <el-button :disabled="scope.row.bizFlowState=='1'||scope.row.bizFlowState=='2'" type="text"
                       @click="sendToProcessApprova( scope.row,scope.$index)"
                       icon="el-icon-s-promotion" title="发审">发审</el-button>
    ...
    <el-row>
      <!--新增修改明细 StockIn pur_stock_in界面-->
      <mdp-dialog :ref="refId+'FormDialog'">
        <template v-slot="{visible,data,dialog}">
          <stock-in-form :ref="refId+'Form'" :visible="visible" :parent-op-type="currOpType"
...
import StockInForm from './Form';//新增修改明细界面
...
export default {
  ...
  data() {
    return {
      ...
      menuDefName: 'pur_stock_in',//menu_def.name 功能名称，用于导出excel等文件名
      refId: 'stockIn',//引用编号，<table :ref="refId+'Table'"> <form :ref="refId+'Form'">
      ...
      //增删改查(含批量)接口
      apis: {
        list: StockInApi.listStockIn,
        add: StockInApi.addStockIn,
        del: StockInApi.delStockIn,
        edit: StockInApi.editStockIn,
        editSomeFields: StockInApi.editSomeFieldsStockIn,
        batchAdd: StockInApi.batchAddStockIn,
        batchDel: StockInApi.batchDelStockIn,
        batchEdit: StockInApi.batchEditStockIn,
      }
    }
  },
  methods: {
    ...
    sendToProcessApprova(row) {
      sendStockInToProcessApprova(this, {stockIn: row})
    },
    //页面初始化需要配置的特殊逻辑写这里
    initCurrData() {
    ...
    preQueryParamCheck(params) {
      params.reqBranchId = this.userInfo.branchId
      if (this.filters.reqDate && this.filters.reqDate.length === 2)
        params.reqDate = `$between ${this.filters.reqDate[0]},${this.filters.reqDate[1]}`;
      return true;
    },
