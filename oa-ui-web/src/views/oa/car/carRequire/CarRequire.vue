			...
			<el-form :model="addForm"  label-width="150px" :rules="addFormRules" ref="addForm">
				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="申请编号" prop="id">
							<el-input v-model="addForm.id" placeholder="可不填,后台自动生成" ></el-input>
						</el-form-item> 
					</el-col>
				</el-row>
				<el-row>
					<el-col :span="12">
						<el-form-item label="申请用车开始时间" prop="reqStartTime">
							<el-date-picker type="date" placeholder="选择日期" v-model="addForm.reqStartTime" value-format="timestamp" format="yyyy-MM-dd"></el-date-picker>
						</el-form-item> 
					</el-col>
					<el-col :span="12">
						<el-form-item label="申请用车结束时间" prop="reqEndTime">
							<el-date-picker type="date" placeholder="选择日期" v-model="addForm.reqEndTime" value-format="timestamp" format="yyyy-MM-dd"></el-date-picker>
						</el-form-item> 
					</el-col>
				</el-row>
				...
				<el-row style="margin-left:150px">
					<el-table :data="addForm.carExpenseDetailTpls" border >
						...
						<el-table-column align="center" label="费用项目编号" min-width="150">
						<template slot-scope="row">
							<el-input v-model="addForm.carExpenseDetailTpls[row.$index].itemId" placeholder="可不填"> </el-input>
						</template>
						</el-table-column>
						<el-table-column align="center" label="费用项目" min-width="200">
						<template slot-scope="row">
							<el-input v-model="addForm.carExpenseDetailTpls[row.$index].itemName" placeholder="费用项目名称"> </el-input>
						</template>
						</el-table-column>
						<el-table-column align="center" label="费用" min-width="120">
						<template slot-scope="row">
							<el-input v-model="addForm.carExpenseDetailTpls[row.$index].feeAmount" placeholder="费用"> </el-input>
						</template>
						</el-table-column>
						<el-table-column align="center" label="费用说明" min-width="200">
						<template slot-scope="row">
	...
	export default { 
		...
		data() {
			return {
				...
				//新增界面数据 car_require
				addForm: {
					reqStartTime: this.getReqStartTime(),reqEndTime: this.getReqEndTime(),id:'',reqDeptid:'',reqDeptName:'',reqBranchId:'',reqUserid:'',reqUsername:'',reqPhoneno:'',reqReason:'',reqTime:'',reqStatus:'',bizFlowState:'',bizProcInstId:'',destAddress:'',destGps:'',reqTitle:'',hasRoute:true,reqBranchName:'',
						...
						this.$confirm('确认提交吗？', '提示', {}).then(() => { 
							...
							addCarRequire(params).then((res) => {
								...
								if(tips.isOk){
									...
									this.addForm.carExpenseDetailTpls = []
									this.addForm.carRouteTpls = ''
...
    		},
			...
			showEditLocation: function(attrName,addr) {
				...
				if(attrName == 'addForm.destGps' ){
					...
					if(this.carRouteTplsNew.length > 0){
						if(addr == '0'){
							console.log(this.carRouteTplsNew[0].fromAddressGps)
							this.value = this.carRouteTplsNew[0].fromAddressGps
							this.addrName = this.carRouteTplsNew[0].fromAddress	
						}else{
							console.log(this.carRouteTplsNew[0].destAddressGps)
		...
		mounted() {
			...
			this.addForm.reqPhoneno = this.userInfo.phoneno
			/**在下面写其它函数***/
		}//end mounted
	}
