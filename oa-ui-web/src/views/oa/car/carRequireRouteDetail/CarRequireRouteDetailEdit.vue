		 ...
		 <el-row class="page-container border padding"> 
			<el-form :model="editForm"  label-width="120px" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="路线编号" prop="routeId">
					<el-input v-model="editForm.routeId" placeholder="路线编号"></el-input>
				</el-form-item> 
				<el-form-item> 
...
<script>
	...
	import { editCarRequireRouteDetail } from '@/api/oa/car/carRequireRouteDetail';
	...
	export default { 
		...
		props:['carRequireRouteDetail','visible'],
		watch: {
	      'carRequireRouteDetail':function( carRequireRouteDetail ) {
	        this.editForm = carRequireRouteDetail;
	      },
		...
		data() {
			return {
				...
				//编辑界面数据  CarRequireRouteDetail car_require_route_detail
				editForm: {
		...
		methods: {
			...
			//编辑提交CarRequireRouteDetail car_require_route_detail父组件监听@submit="afterEditSubmit"
			editSubmit: function () {
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => { 
							...
							editCarRequireRouteDetail(params).then((res) => {
		...
		components: {  
		    //在下面添加其它组件 'car-require-route-detail-edit':CarRequireRouteDetailEdit
		},
		mounted() {
			this.editForm=Object.assign(this.editForm, this.carRequireRouteDetail);  
		}
	}
