<template>
	<section class="page-container border padding">
      ...
      <el-input v-model="filters.key" style="width: 270px;margin-right: 13px;" placeholder="请输入姓名或工号" clearable
        ...
        <el-button slot="append" icon="el-icon-search" :disabled="load.list==true" v-on:click="searchHrUserCredentials">
          查找</el-button>
      </el-input>
      ...
      <el-table :height="tableHeight" ref="procdefsTable"  :data="hrUserCredentials" @sort-change="sortChange" highlight-current-row v-loading="load.list" border
        ...
        <el-table-column prop="certificateName" label="证书名称" show-overflow-tooltip min-width="80"></el-table-column>
        <el-table-column prop="startTime" label="颁发日期" show-overflow-tooltip min-width="80"></el-table-column>
        <el-table-column prop="endTime" label="失效日期" show-overflow-tooltip min-width="80"></el-table-column>
        <el-table-column prop="issueUnit" label="颁发单位" show-overflow-tooltip min-width="80"></el-table-column>
    ...
    <el-dialog title="编辑资格证书" :visible.sync="editFormVisible" width="60%" :close-on-click-modal="false">
      <hr-user-credential-edit :hr-user-credential="editForm" :visible="editFormVisible" @cancel="editFormVisible=false"
        @submit="afterEditSubmit"></hr-user-credential-edit>
    </el-dialog>
  </section>
</template>
...
<script>
  ...
  import {
    listHrUserCredential,
    delHrUserCredential,
    batchDelHrUserCredential
  } from "@/api/oa/hr/user/hrUserCredential";
  import {
    mapGetters
  } from "vuex";
  ...
  import HrUserCredentialEdit from "@/views/oa/hr/user/hrUserCredential/HrUserCredentialEdit"; //修改界面
  const REGULAR = {
    ...
    data() {
      return {
        ...
        uploadApi: config.getExcelUploadBasePath() +
          "/oa/hr/user/hrUserCredential/addCredentialByExcel",
        header: [
          "组织架构编号",
          "系统账号",
          "工号",
    ...
    methods: {
      ...
      batchDel: function() {
        this.$confirm("确认删除选中记录吗？", "提示", {
          ...
          this.load.del = true;
          batchDelHrUserCredential(this.sels)
            .then(res => {
              this.load.del = false;
              var tips = res.data.tips;
              if (tips.isOk) {
      ...
      validator(row, arr, i) {
        ...
        if (row.证书名称 == "" || typeof row.证书名称 == "undefined") {
          return `<span class="c-red pdl10">第${i +
          1}行，【证书名称】 不能为空 </span></br>`;
        }

        if (!this.isNull(row.颁发日期) && !REGULAR.DATA.test(row.颁发日期)) {
          return `<span class="c-red pdl10">第${i +
          1}行，【颁发日期】 时间格式不正确，时间格式应该为 【yyyy-MM-dd】</span></br>`;
        }
        if (!this.isNull(row.失效日期) && !REGULAR.DATA.test(row.失效日期)) {
          return `<span class="c-red pdl10">第${i +
          1}行，【失效日期】 时间格式不正确，时间格式应该为 【yyyy-MM-dd】</span></br>`;
        }
        return true;
      },
      importSuccess() {
