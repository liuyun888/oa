<template>
	<section class="page-container border padding">
      ...
      <el-input v-model="filters.key" style="width: 270px;margin-right: 13px;" placeholder="请输入姓名" clearable
        ...
        <el-button slot="append" icon="el-icon-search" :disabled="load.list==true" v-on:click="searchHrUserTrains">查找
        </el-button>
      </el-input>
      ...
      <el-table :height="tableHeight" ref="procdefsTable"  :data="hrUserTrains" @sort-change="sortChange" highlight-current-row v-loading="load.list" border
        ...
        <el-table-column prop="trainName" label="培训名称" show-overflow-tooltip min-width="80"></el-table-column>
        <el-table-column prop="startTime" label="开始日期" show-overflow-tooltip min-width="80"></el-table-column>
        <el-table-column prop="endTime" label="结束日期" show-overflow-tooltip min-width="80"></el-table-column>
        <el-table-column prop="trainUnit" label="培训单位" show-overflow-tooltip min-width="80"></el-table-column>
    ...
    <el-dialog title="编辑培训记录" :visible.sync="editFormVisible" width="60%" :close-on-click-modal="false">
      <hr-user-train-edit :hr-user-train="editForm" :visible="editFormVisible" @cancel="editFormVisible=false"
        @submit="afterEditSubmit"></hr-user-train-edit>
    </el-dialog>
  </section>
</template>
...
<script>
  ...
  import {
    ...
    delHrUserTrain,
    batchDelHrUserTrain
  } from "@/api/oa/hr/user/hrUserTrain";
  import {
    mapGetters
  } from "vuex";
  ...
  import HrUserTrainEdit from "@/views/oa/hr/user/hrUserTrain/HrUserTrainEdit"; //修改界面
  const REGULAR = {
  ...
  export default {
    ...
    data() {
      return {
        ...
        uploadApi: config.getExcelUploadBasePath() +
          "/oa/hr/user/hrUserTrain/addTrainByExcel",
        header: [
          "组织架构编号",
          "系统账号",
          "工号",
    ...
    methods: {
      ...
      validator(row, arr, i) {
        ...
        if (row.培训名称 == "" || typeof row.培训名称 == "undefined") {
          return `<span class="c-red pdl10">第${i +
          1}行，【培训名称】 不能为空 </span></br>`;
        }
        ...
        if (!!this.isNull(row.结束日期) && !REGULAR.DATA.test(row.结束日期)) {
          return `<span class="c-red pdl10">第${i +
          1}行，【结束日期】 时间格式不正确，时间格式应该为 【yyyy-MM-dd】</span></br>`;
        }
        return true;
      },
      importSuccess() {
