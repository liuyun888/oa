<template>
	<div>
		...
		<el-row style="margin-top: 30px" class="container"> 
            <!--列表 ContractInvoice ht_contract_invoice-->
            <el-table :data="contractInvoicesOpen" @sort-change="sortChange" highlight-current-row v-loading="load.list" border @selection-change="selsChange" @row-click="rowClick" style="width: 100%;">
...
<script>
	...
	export default { 
        ...
        props: ['receiveId','htId','showSelect','conditionId'],
		...
		methods: { 
			...
			getContractInvoicesOpen() {
				...
				if(this.receiveId){
					params.receiveId=this.receiveId
				}
				if(this.htId){
				...
				if(!params.receiveId && !params.htId && !params.conditionId){
					return;
				}
				this.load.list = true;
