<template>
	<section class="page-container border padding"> 
		<el-row class="padding-top">
			<!--列表 ContractProject 项目合同关联关系表-->
			<el-table ref="contractProjectTable" :data="contractProjects" :height="maxTableHeight" highlight-current-row v-loading="load.list" border  @row-click="rowClick" style="width: 100%;">
				...
				<el-table-column prop="projectId" label="项目编号" min-width="80"> 
					<template slot-scope="row">
						<el-input size="mini"  v-model="contractProjects[row.$index].projectId"> </el-input>
					</template>
				</el-table-column>
				<el-table-column prop="projectName" label="项目名称" min-width="80"> 
					<template slot-scope="row">
						<el-input size="mini"  v-model="contractProjects[row.$index].projectName"> </el-input>
					</template>
				</el-table-column>
...
<script>
	...
	import { batchSaveProject,listContractProject } from '@/api/oa/ht/contractProject';
	...
	export default { 
		...
		methods: {
			...
			editSubmit: function () { 
				this.$confirm('确认提交吗？', '提示', {}).then(() => { 
					...
					let params={
						...
						contractProjects:this.contractProjects
					}
					batchSaveProject(params).then((res) => {
						this.load.edit=false
						var tips=res.data.tips;
						if(tips.isOk){
							this.searchContractProjects();
			...
			addRow(type) {
				switch(type) {
					case 'pro':
						var date=util.formatDate(new Date(),'yyyy-MM-dd HH:mm:ss')
					this.contractProjects.push( 
						{
							id:'',htId:'',projectId:'',projectName:'',ctime:date,ltime:date
						},
					)
					break;
					case 'thirdParty' :
			...
			delRow(type, index) {
				switch(type) {
					case 'pro':
					this.contractProjects.splice(index, 1);
					break;
					case 'thirdParty' :
			...
			searchContractProjects(){
				 this.pageInfo.count=true; 
				 this.getContractProjects();
			},
			//获取列表 ContractProject 项目合同关联关系表
			getContractProjects() {
				let params = {
				...
				this.load.list = true;
				listContractProject(params).then((res) => {
					var tips=res.data.tips;
					if(tips.isOk){ 
						this.pageInfo.total = res.data.total;
						this.pageInfo.count=false;
						this.contractProjects = res.data.data;
					}else{
