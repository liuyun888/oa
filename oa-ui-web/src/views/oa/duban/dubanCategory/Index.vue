<template>
  <section class="page-container border padding">
    <el-row>
      <el-input v-model="filters.id" style="width: 20%;" placeholder="主键查询 输入 *字符* >10 <9 等" clearable
                title="支持>、<、 >=、<=、!=、*字符*、$IS NULL、$IN 1,2,3、$between 1,5等操作符"/>
      <el-input v-model="filters.branchId" style="width: 20%;" placeholder="归属机构查询 输入 *字符* >10 <9 等" clearable
                title="支持>、<、 >=、<=、!=、*字符*、$IS NULL、$IN 1,2,3、$between 1,5等操作符"/>
    ...
    <el-row>
      ...
      <el-table :ref="refId+'Table'" :height="100" v-adaptive="{bottomOffset: bottomOffset}" :data="tableDatas"
        ...
        <el-table-column prop="id" label="主键" min-width="120" show-overflow-tooltip fixed="left" col-type="String"
                         v-if="showCol('id')"></el-table-column>
        <el-table-column prop="name" label="分类名称" min-width="120" show-overflow-tooltip col-type="String"
                         v-if="showCol('name')">
        </el-table-column>
        ...
        <el-table-column prop="status" label="状态" min-width="120" show-overflow-tooltip col-type="String"
          ...
          <template slot-scope="scope">
            <mdp-select item-code="duban_category_status" show-style="tag" v-model="scope.row.status" :maxlength="1"
                       @change="editSomeFields(scope.row,'status',$event)"
                       :disabled="!editable || disabledJudge('status')"/>
          </template>
        </el-table-column>

        <el-table-column prop="branchId" label="归属机构" min-width="120" show-overflow-tooltip col-type="String"
                         v-if="showCol('branchId')">
          <template slot-scope="scope">
            <mdp-input show-style="tag" v-model="scope.row.branchId" :maxlength="50"
                       @change="editSomeFields(scope.row,'branchId',$event)"
                       :disabled="!editable || disabledJudge('branchId')"/>
          </template>
        </el-table-column>
        <el-table-column prop="remark" label="备注说明" min-width="120" show-overflow-tooltip col-type="String"
                         v-if="showCol('remark')">
          <template slot-scope="scope">
            <mdp-input show-style="tag" v-model="scope.row.remark" :maxlength="50"
                       @change="editSomeFields(scope.row,'remark',$event)"
                       :disabled="!editable || disabledJudge('remark')"/>
          </template>
        </el-table-column>
        <el-table-column prop="createTime" label="新增时间" min-width="120" show-overflow-tooltip col-type="Date"
                         v-if="showCol('createTime')">
          <template slot-scope="scope">
            <mdp-date type="date" placeholder="选择日期" show-style="tag" v-model="scope.row.createTime"
                      @change="editSomeFields(scope.row,'createTime',$event)" value-format="yyyy-MM-dd HH:mm:ss"
                      format="yyyy-MM-dd" :disabled="!editable || disabledJudge('createTime')"/>
          </template>
        </el-table-column>
    ...
    <el-row>
      <!--新增修改明细 DubanCategory 督办分类表界面-->
      <mdp-dialog :ref="refId+'FormDialog'">
        <template v-slot="{visible,data,dialog}">
          <duban-category-form :ref="refId+'Form'" :visible="visible" :parent-op-type="currOpType"
...
import DubanCategoryForm from './Form';//新增修改明细界面
...
export default {
  ...
  data() {
    return {
      ...
      menuDefName: '督办分类表',//menu_def.name 功能名称，用于导出excel等文件名
      refId: 'dubanCategory',//引用编号，<table :ref="refId+'Table'"> <form :ref="refId+'Form'">
      ...
      //增删改查(含批量)接口
      apis: {
        list: DubanCategoryApi.listDubanCategory,
        add: DubanCategoryApi.addDubanCategory,
        del: DubanCategoryApi.delDubanCategory,
        edit: DubanCategoryApi.editDubanCategory,
        editSomeFields: DubanCategoryApi.editSomeFieldsDubanCategory,
        batchAdd: DubanCategoryApi.batchAddDubanCategory,
        batchDel: DubanCategoryApi.batchDelDubanCategory,
        batchEdit: DubanCategoryApi.batchEditDubanCategory,
      }
    }
  },
  methods: {
