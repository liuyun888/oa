<template>
	<section>
		...
		<!--编辑界面 MatterInspectStandard 事项检查依据--> 
			<el-form :model="editForm" size="small" label-width="120px" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="依据名称（检查依据）" prop="accordingName">
					<el-input v-model="editForm.accordingName" auto-complete="off"></el-input>
				</el-form-item> 
				<el-form-item label="依据条款（检查依据）" prop="clause">
					<el-input v-model="editForm.clause" auto-complete="off"></el-input>
				</el-form-item> 
				<el-form-item label="检查内容id" prop="contentId">
					<el-input v-model="editForm.contentId" auto-complete="off"></el-input>
				</el-form-item> 
...
<script>
	...
	import { editMatterInspectStandard } from '../../../../api/oa/matter/matterInspectStandard';
	
	export default {
		props:['matterInspectStandard','visible'],
		watch: {
	      'matterInspectStandard':function( matterInspectStandard ) {
	        this.editForm = matterInspectStandard;
	      },
		...
		data() {
			return {
				...
				//编辑界面数据  MatterInspectStandard 事项检查依据
				editForm: {
		...
		methods: {
			...
			//编辑提交MatterInspectStandard 事项检查依据父组件监听@submit="afterEditSubmit"
			editSubmit: function () {
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							editMatterInspectStandard(params).then((res) => {
		...
		mounted() {
			this.editForm=Object.assign(this.editForm, this.matterInspectStandard);
			//加载下拉列表 如有需要去掉注释
			/**
