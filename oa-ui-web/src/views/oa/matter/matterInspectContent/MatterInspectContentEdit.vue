<template>
	<section>
		...
		<!--编辑界面 MatterInspectContent 事项检查内容--> 
			<el-form :model="editForm" size="small" label-width="120px" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="检查内容名称" prop="contentName">
					<el-input v-model="editForm.contentName" auto-complete="off"></el-input>
				</el-form-item> 
				<el-form-item label="检查方法" prop="inspectMethod">
					<el-input v-model="editForm.inspectMethod" auto-complete="off"></el-input>
				</el-form-item> 
				<el-form-item label="法定频次（检查频次）" prop="legalFrequency">
					<el-input v-model="editForm.legalFrequency" auto-complete="off"></el-input>
				</el-form-item> 
				<el-form-item label="建议频次（检查频次）" prop="adviseFrequency">
					<el-input v-model="editForm.adviseFrequency" auto-complete="off"></el-input>
				</el-form-item> 
...
<script>
	...
	import { editMatterInspectContent } from '../../../../api/oa/matter/matterInspectContent';
	
	export default {
		props:['matterInspectContent','visible'],
		watch: {
	      'matterInspectContent':function( matterInspectContent ) {
	        this.editForm = matterInspectContent;
	      },
		...
		data() {
			return {
				...
				//编辑界面数据  MatterInspectContent 事项检查内容
				editForm: {
		...
		methods: {
			...
			//编辑提交MatterInspectContent 事项检查内容父组件监听@submit="afterEditSubmit"
			editSubmit: function () {
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							editMatterInspectContent(params).then((res) => {
		...
		mounted() {
			this.editForm=Object.assign(this.editForm, this.matterInspectContent);
			//加载下拉列表 如有需要去掉注释
			/**
