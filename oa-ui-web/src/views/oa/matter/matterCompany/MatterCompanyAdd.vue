		...
		<el-row>
			...
			<el-form :model="addForm" size="small" label-width="120px" :rules="addFormRules" ref="addForm">
				...
				<el-form-item label="事项id" prop="matterInfoId">
					<el-input v-model="addForm.matterInfoId" auto-complete="off"></el-input>
				</el-form-item> 
...
<script>
	...
	import { addMatterCompany } from '../../../../api/oa/matter/matterCompany';
	
	export default {
		props:['matterCompany','visible'],
		watch: {
	      'matterCompany':function( matterCompany ) {
	        this.addForm = matterCompany;
	      },
		...
		methods: {
			...
			//新增提交MatterCompany 事项与公司关联表 父组件监听@submit="afterAddSubmit"
			addSubmit: function () {
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							addMatterCompany(params).then((res) => {
		...
		mounted() {
			this.addForm=Object.assign(this.addForm, this.matterCompany);
			//加载下拉列表 如有需要去掉注释
			/**
